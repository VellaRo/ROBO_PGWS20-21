...
void Just_move::calculateCommand() {
	...
	// the angular velocity (around z axis, yaw) is measured in rad/sec
 	...
 	if( (&m_laserscan)->ranges.size() > 0) 
	  {
	  //Laserwerte für Formel
	  float right_laser;
	  float left_laser;
	  
	  	
	       
	       right_laser =m_laserscan.ranges[120];
	   
	       left_laser =m_laserscan.ranges[420];
	       
	      
	       // bei kleinem abstand
	       if(m_laserscan.ranges[270] < 2.0){
	       
	      			 right_laser =m_laserscan.ranges[250];
	      		
	      			 left_laser =m_laserscan.ranges[290];
	       
	       	
	       } // end if // bei kleiem abstand
	  		
	  		
	  		//FORMEL
	  		m_roombaCommand.linear.x  = 0.3* tanh(0.5*(right_laser + left_laser)-1.20);		
			m_roombaCommand.angular.z  = 0.3* tanh((right_laser-left_laser));	   	 
...
void Just_move::mainLoop() {
	...
	while (m_nodeHandle.ok())
	{
	  ...
	  /* Plan  */
		...
		ROS_INFO(" robo_I movement commands:forward speed=%f[m/sec] and turn speed =%f[rad/sec]", 

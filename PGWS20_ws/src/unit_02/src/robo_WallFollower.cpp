...
void Just_move::followWallLeft(){
		// abstand von 0.50 m halten

		//double speedVar =0.5+m_laserscan.ranges[450];
	  	m_roombaCommand.linear.x  = 0.2* tanh(0.5*( m_laserscan.ranges[250] +  m_laserscan.ranges[290])-0.30);		
		m_roombaCommand.angular.z = 0.3 * tanh(m_laserscan.ranges[440] - m_laserscan.ranges[460]);	

		if(m_laserscan.ranges[450] < 0.48){
			m_roombaCommand.angular.z = -0.2;
		}
		else if(m_laserscan.ranges[450] > 0.52){
			m_roombaCommand.angular.z = 0.2;
		}
		
		//Linke seite abstand prüfen
		...
		// stehnbleiben zum drehen
		for(int i =190;i<350;i++){
			if(m_laserscan.ranges[i] < 0.35 ){
				m_roombaCommand.linear.x  = 0.0;
		}
		}
...
void Just_move::mainLoop() {
	...
	while (m_nodeHandle.ok())
	{
	  ...
	  /* Plan  */
		...
		ROS_INFO(" robo_WallFollower movement commands:forward speed=%f[m/sec] and turn speed =%f[rad/sec]", 

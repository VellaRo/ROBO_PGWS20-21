...
void poseAMCLCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msgAMCL){
	...
	//als Euler (Matrix auslesen in r ,p,y schreiben)
...
void Mystery_mover::mainLoop() {
	...
	while (m_nodeHandle.ok())
	{
	   ...
	   /* Plan  */
		...
		ROS_INFO(" robo_AMCL_Test3 commands:forward speed=%f[m/sec] and turn speed =%f[rad/sec]  \n poseAMCLx %f \n poseAMCLy %f \n poseAMCLZ %f \n	\n poseOdomX %f \n poseOdomY %f \n poseOdomZ %f \n\n Orientierung in Euler(ODOM) R P M: %.3f %.3f %.3f \n Orientierung in Euler(AMCL) R P M: %.3f %.3f %.3f" , 
			m_roombaCommand.linear.x, m_roombaCommand.angular.z ,
			amclPositionX ,amclPositionY ,amclPositionZ,	
			odomPositionX , odomPositionY , odomPositionZ,
			amclRoll, amclPitch, amclYaw,
			odomRoll,odomPitch,odomYaw );

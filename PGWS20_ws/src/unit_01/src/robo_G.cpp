 	...
 	if( (&m_laserscan)->ranges.size() > 0) 
	  {
	     // go through some laser beams on the right
             for(unsigned int i=260 ; i < 280 ; i++)
	       {
                 // is there something?
	         if (m_laserscan.ranges[i] < 1.20)
	           {
                     m_roombaCommand.linear.x = 0.0;
		     m_roombaCommand.angular.z = 0.0;
...
void Just_move::mainLoop() {
	...
	while (m_nodeHandle.ok())
	{
	  ...
	  /* Plan  */
		...
		ROS_INFO(" robo_G movement commands:forward speed=%f[m/sec] and turn speed =%f[rad/sec]", 
